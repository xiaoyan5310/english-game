...
function gamePvz(data) {
  ...
  // 僵尸类型配置
  var ZOMBIE_TYPES = {
    normal: { icon: '🧟', hp: 2, speedMul: 1.0, name: '普通僵尸', color: '#84CC16' },
    cone:   { icon: '🧟', hp: 4, speedMul: 0.9, name: '路障僵尸', color: '#F59E0B' },
    bucket: { icon: '🧟', hp: 7, speedMul: 0.75, name: '铁桶僵尸', color: '#6B7280' },
    flag:   { icon: '🧟', hp: 10, speedMul: 0.65, name: '旗手僵尸', color: '#EF4444' }
  };
  ...
  function spawnZombie() {
    ...
    spawnZombieOnLane(lane, w);
  }

  // 在指定行生成僵尸（用于多行同时出僵尸）
  function spawnZombieOnLane(lane, forcedWord) {
    if (gameOver) return;
    if (words.length === 0) return;

    var available = words.filter(function(w) { return !activeWords[w.en]; });
    if (available.length === 0) available = words;
    var w = forcedWord || available[Math.floor(Math.random() * available.length)];

    // 根据波次决定僵尸类型概率
    var roll = Math.random();
    var typeKey = 'normal';
    if (wave >= 6 && roll < 0.08) typeKey = 'flag';
    else if (wave >= 4 && roll < 0.20) typeKey = 'bucket';
    ...
    var zObj = {
      uid: uid, el: zombie, lane: lane, word: w.en, answer: w.zh, spd: spd, alive: true,
      hp: type.hp, maxHp: type.hp, type: typeKey, typeName: type.name
    };
    zombies.push(zObj);
  ...
  function startWave() {
    ...
    var count = Math.min(1 + Math.floor(wave / 2), 3); // 每波1~3只，密度再降低
    var usedLanes = {};
    ...
    // 波结束后暂停2秒
    var waveDuration = 6000 + count * 1200;
  ...
  window.pvzAttack = function(chosen) {
    ...
    if (lp && lp.chomp && z.hp <= 0) {
      bonus = 10 + (z.type === 'bucket' || z.type === 'flag' ? 10 : 0);
      playBigKillSound();
      document.getElementById('pvzInfo').textContent = '🌺 大嘴花一口吞掉' + z.typeName + '！+' + bonus + '☀️';
    } else if (lp && lp.bomb) {
      playBigKillSound();
      ...
      if (z.hp <= 0) {
        ...
        if (lp && lp.attack) {
          ...
          document.getElementById('pvzInfo').textContent = '✅ 消灭' + z.typeName + '！+' + bonus + '☀️';
        }
      } else {
        // 僵尸受伤但没死
        z.el.style.animation = 'shake 0.2s ease';
        setTimeout(function() { z.el.style.animation = 'pvzWalk ' + z.spd + 's linear forwards'; }, 200);
        document.getElementById('pvzInfo').textContent = '💢 ' + z.typeName + '受伤了！（剩' + z.hp + '血）';
      }
    }
  ...
  function updateZombieHpBar(z) {
    var bar = document.getElementById('hpBar' + z.uid);
    if (!bar) return;
    var pct = Math.max(0, z.hp / z.maxHp * 100);
    bar.style.width = pct + '%';
  }
